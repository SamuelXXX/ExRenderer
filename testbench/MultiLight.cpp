...
namespace ExRenderer::Testbench::MultiLight
{
    struct VertexData
    {
        Vector3 position;
    ...
    class DiffuseShaderAdditionalPass:public Shader<VertexData,FragmentData>
    {
        Vector4 baseColor;

    public:
        DiffuseShaderAdditionalPass()
        {
            zTest = ZTestType::LessEqual;
            zWrite = true;
            doubleSide=false;
    ...
    public:
        ...
        Vector4 FragmentShader(const FragmentData &fragment) override
        {
            ...
            Vector4 color=GetLightColor(fragment.worldPos)*diffuse;
            color.x*=baseColor.x;
            color.y*=baseColor.y;
            color.z*=baseColor.z;
    ...
    void UpdateRenderer(ForwardPipelineRenderer &renderer)
    {
        ...
        static DiffuseShaderAdditionalPass diffShaderMultiPass;
        ...
        diffShaderMultiPass.SetBaseColor(Vector4(0.3,0.7,0.2,1));

        renderer.RenderMesh(sphereMesh,diffShader);
        renderer.RenderMeshMultiLightPass(sphereMesh,diffShaderMultiPass);
    }

    void Test()
    {
        ForwardPipelineRenderer fRenderer("MultiLight",1920, 1080);
        while (true)
        {
